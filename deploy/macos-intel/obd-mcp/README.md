...
A self-contained build of the `obdium` MCP server for **Intel macOS (x86_64)**.
...
```
obd-mcp/
├── obd-mcp                       the MCP server (x86_64 binary)
├── data/
...
- **Intel macOS.** This binary is `x86_64-apple-darwin`. It runs natively on
  Intel Macs and under Rosetta on Apple Silicon; for a native Apple Silicon build
  use the `macos-arm64` package instead.
