...
# Safe to re-run. Requires: macOS on Apple Silicon (arm64).
set -euo pipefail
...
arch="$(uname -m)"
if [[ "$arch" != "arm64" ]]; then
  echo "warning: this binary is built for Apple Silicon (arm64) but this machine is $arch." >&2
  echo "         It may run under Rosetta, or you may need to rebuild from source." >&2
fi
