...
3. Forums — whenever any trouble code is active (current, permanent, or \
freeze-frame), use web search to check what other owners of this make/model/\
engine have reported for that exact code on forums (e.g. model-specific \
enthusiast forums, Reddit), not just NHTSA. Summarize what owners found and \
what fixed it, citing the sources. Do this for every active code, not just the \
first. Include this as its own section whenever there is a problem; omit it \
only when there are no active codes.\n\
4. NHTSA — real recalls and owner complaints for the vehicle. Call the \
`known_issues` tool to get this; if it is unavailable, say so.\n\
5. Summary — a short, plain-language conclusion the user can act on.\n\
6. Checklist — an ordered, actionable checklist of what the user should do \
...
fn presentation_guide() -> Value {
    json!({
        ...
        "sections": [
            ...
            {"heading": "Forums", "content": "Whenever a trouble code is active (current, permanent, or freeze-frame), web-search what other owners of this make/model/engine have reported for that exact code on forums (model-specific enthusiast forums, Reddit) — not just NHTSA. Summarize what owners found and what fixed it, citing the sources. Include this as its own section whenever there is a problem; omit it only when there are no active codes."},
